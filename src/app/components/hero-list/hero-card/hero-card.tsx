...
export default function HeroCard({ hero }: HeroCardProps) {
    ...
    const onClickFavorite = (event: React.MouseEvent<HTMLInputElement>) => {
      event.preventDefault()
      let newFavorites: FavoriteHero[] = [];

      if (hero.isFavorite) {
        newFavorites = [...favorites.filter(favorite => favorite.id !== hero.id)]
      }

      newFavorites = [...favorites, { ...hero}]
            ...
            {hero.isFavorite ? (
              <i onClick={onClickFavorite}>
                <svg
                  ...
                  className={`${styles.iconIsFavorite}`}
                >
                  <path
                    fillRule='evenodd'
                    clipRule='evenodd'
              ...
              <i onClick={onClickFavorite}>
                <svg
                  ...
                  fill='none'
                  xmlns='http://www.w3.org/2000/svg'
                  className={`${styles.iconNoneIsFavorite}`}
                >
                  <path
